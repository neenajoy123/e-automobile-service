...
<html>
    <head>
        ...
        <script>
            ...
            {
                          alert(v);
             $.ajax({url: "ajax_spModelSale.jsp?id="+v, success: function(result){
        $("#model").html(result);
    }});
            }
            ...
            {
                          alert(v);
             $.ajax({url: "ajax_SparePartsSale.jsp?id="+v, success: function(result){
            ...
            function getPrice(u)
        {
            ...
            $.ajax(
                    {
                        url: "AjaxgetspSalePrice.jsp?id="+ u,
                        success: function(result)
                        {
                          $("#price").html(result);
                        }
...
//            var qun=document.getElementById("qun").value;
            var des=document.getElementById("txtdes").value;
              $.ajax(
                    {
                        url: "ajax_sparepartsSaleInsert.jsp",
                        data: {invoice:inv,date1:date,cus:cus,des:des},
                        success: function(result)
                        {
                          insertItems();
                        }
    ...
    <body>
        ...
        <h1><center>Spare parts Sale</center></h1>
        <form>
            <%
            //.
             if(request.getParameter("btnsave")!=null)
                {
                    ...
                    String update="update tbl_spartssalehead set ssh_gtotal='"+total+"' where ssh_id="+session.getAttribute("sph");
            ...
            // Creating Invoice No
            String no="0";
            String sel="select MAX(ssh_invoiceno) as invoice from tbl_spartssalehead where branch_id="+session.getAttribute("Branch");
